...
class TestConfigFromFile:
    ...
    def test_kwargs_override_file_config(self):
        ...
        try:
            cfg = config.Config(config_file=config_file, debug=True,
                              log_file='/override/path.log')
...
class TestParseArguments:
    ...
    def test_parse_services_list(self):
        ...
        with patch('sys.argv', ['prog', '--services', 'svc1.service',
                               'svc2.service']):
            cfg = config.parse_arguments()
    ...
    def test_parse_multiple_arguments(self):
        ...
        with patch('sys.argv', ['prog', '--debug', '--log-file', '/tmp/test.log',
                               '--poll-interval', '300']):
            cfg = config.parse_arguments()
            assert cfg.debug is True
            assert cfg.log_file == '/tmp/test.log'
